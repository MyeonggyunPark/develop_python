# 독일생활 고양이 6컷 웹툰 자동화 에이전트 설계안

## 문서 범위
- 이 문서는 **독일생활 고양이 6컷 웹툰 자동화 에이전트** 단일 에이전트의 설계만 다룬다.
- 여러 자동화 에이전트를 통합 관리하는 웹 플랫폼 설계는 별도 문서에서 관리한다.

## 목표
- **입력 이미지(검은 고양이 + 회색 고양이)**와 **기본 프롬프트**는 항상 고정한다.
- 사용자가 주간 기준일에 주제를 입력하거나 추천 주제를 선택하면, 그 뒤의 6컷 웹툰 생성 및 Drive 업로드 파이프라인을 자동 진행한다.
- 최종 이미지와 대화/영상 프롬프트 JSON을 함께 생성하고 품질 검수를 수행한다.

## 성공 기준
### MVP 성공 기준
- 사용자가 주제를 직접 입력하면 해당 주제로 6컷 기획 초안과 웹툰 산출물을 생성할 수 있다.
- 사용자가 `"주제 추천해줘"`라고 요청한 경우에만 기존 실행 이력과 비교하여 중복 가능성이 낮은 주제 후보를 추천할 수 있다.
...
- 최종 승인본 기준으로 이미지 품질과 JSON 산출물 정합성이 모두 확보된다.
- 실행 이력, 사용한 주제, 추천 여부, 최종 승인 결과가 모두 기록된다.

## 전체 구조 (3 에이전트 + 오케스트레이터)

### 0) 오케스트레이터 (스케줄/상태관리)
- 실행 주기: 주 1회 기준일 관리 + 리마인드 알림
- 역할
  - 파이프라인 시작/종료
  - 주제 입력 마감 스케줄 관리
  - 리마인드 알림 및 지연 알림 발송
...
- 주제가 확정되면 이후 기획 생성, 이미지 생성, JSON 산출물 생성, 승인 파이프라인을 자동 진행한다.
...
- `skipped` 처리된 주차는 생성 파이프라인을 시작하지 않고, 메타데이터에 사유를 기록한다.
...
- `script_generated`: 6컷 기획과 캡션 초안 생성이 완료된 상태
- `script_review`: 사용자가 6컷 기획과 캡션 초안을 검토하는 상태
- `script_approved`: 이미지 생성 가능한 스크립트가 확정된 상태
- `image_generated`: 웹툰 이미지 생성이 완료된 상태
- `corrected`: 대화/영상 프롬프트 JSON 생성이 완료된 상태
- `editing`: 사용자가 수동 수정 중인 상태
- `approved`: 최종 승인 완료 상태
- `rejected`: 최종 검토에서 산출물 사용 불가로 반려된 상태
...
11. `topic_selected -> script_generated -> script_review -> script_approved -> image_generated -> corrected -> editing -> approved`
...
- 대화/영상 프롬프트 JSON 생성 실패: 2회 자동 재시도 후 실패 처리
...
- 같은 주차에는 여러 `run_id`가 존재할 수 있지만, 최종 승인 대상은 1개의 `active` 실행만 유지한다.
...
4. 최종 승인 직후 사용할 실행을 `is_active=true`로 확정한다.
5. 같은 `week_key`의 다른 실행은 모두 `is_active=false`로 유지한다.
6. `approved` 상태이면서 `is_active=true`인 실행만 최종 산출물 기준 실행으로 유지한다.
...
- 기존 기획 이력 메타데이터(`run_metadata.json` 집합 또는 별도 인덱스)
...
- `created_at`: 실행 생성 시각
- `status`: 승인/보류/실패 여부
- 추천 비교 대상은 `status=approved` 또는 `status=review_required` 이력만 우선 사용한다.
...
1. 6컷 기획 및 캡션 초안 생성
   ...
   - 2~5컷: 전개/상황 심화
   - 6컷: 마무리 payoff
2. 생성된 6컷 기획과 캡션 초안을 사용자에게 먼저 검토 요청
3. 사용자는 승인, 재생성 요청, 직접 수정 중 하나를 선택한다.
4. 승인된 기획 기준으로 이미지 생성 시 캐릭터 일관성 강제
   - 고정 참조 이미지 + 고정 베이스 프롬프트 항상 포함
   - 스타일 프롬프트(귀여운 벡터풍, 표정 강조, 말풍선 없는 이미지)
5. 컷별 대사 길이는 이미지 안에 넣지 않고, 별도 JSON으로 분리해도 자연스럽게 이어질 정도의 밀도로 유지한다.
6. 생성 에이전트의 기본 출력 파일은 `thumbnail_*`, `panel_*` 계열 이미지와 JSON 산출물로 저장한다.

#### 중간 승인 체크포인트
- 고비용 이미지 생성 전에 사용자가 6컷 기획과 캡션 초안을 먼저 검토한다.
...
- `run_id`: 실행 식별자
- `topic`: 최종 주제
- `caption_draft`: 에피소드 설명 캡션 초안
- `panels`: 6컷 배열
  - `panel_no`: 컷 번호
  - `story_role`: 장면의 서사 역할
  - `location`: 장소
  - `scene_prompt`: 이미지 생성용 장면 설명
  - `key_props`: 핵심 소품
  - `carryover_props`: 다음 컷까지 이어지는 연속 소품
  - `speaker_dialogues`: 등장인물별 대사 배열
  ...
  "panels": [
    {
      ...
      "story_role": "도입",
      "location": "독일 마트 입구",
      "scene_prompt": "마트 입구의 판트 기계 앞에서 콜라와 제로가 빈 병을 들고 어리둥절해하는 장면",
      "key_props": ["빈 병", "판트 기계"],
      "carryover_props": ["빈 병"],
      "speaker_dialogues": [
        {
          "speaker": "kolla",
          "dialogue_lines": ["이 병도 돈으로 바뀐다고?"]
        },
        {
          "speaker": "zero",
          "dialogue_lines": ["일단 기계부터 읽어보자."]
        }
      ]
    }
  ]
...
- `STYLE_FIXED_PROMPT`: 6컷 에피소드 구조, 한국어 대사, 웹툰 패널 이미지 비율
...
**핵심 로직**
1. 컷별 `speaker_dialogues`를 그대로 분리 저장한다.
2. 두 등장인물의 화자와 대사 줄을 정확히 매핑한다.
3. 각 패널의 `scene_prompt`, 장소, 소품 연속성을 바탕으로 영상 프롬프트를 작성한다.
4. 영상 프롬프트에는 이미지에 없는 새 등장인물, 새 소품, 말풍선, 자막, UI를 넣지 않도록 제한한다.
...
#### 구현 원칙
- 생성 모델 출력 이미지는 처음부터 텍스트 없는 컷 이미지 자산으로 보관한다.
- 이미지 안에는 말풍선, 자막, 캡션을 넣지 않는다.
- 대사와 영상 프롬프트는 별도 JSON 파일로 관리한다.
- 영상 프롬프트는 해당 패널 이미지를 기준 프레임으로 사용하는 후속 비디오 생성 AI 입력을 목표로 한다.

## 데이터 저장소 설계 (Google Drive 중심)
### 1) 이미지 및 JSON 저장: Google Drive 폴더 구조
- 루트 폴더 예시: `GermanLifeCatWebtoon`
- 하위 구조(연/월/실행):
  - `{YYYY}/`
    - `{MM}/`
      - `{run_id}/`
        - `thumbnail_base_v1.png`
        - `thumbnail_final_v1.png`
        - `thumbnail_publish_v1.jpg`
        - `panel_01_base_v1.png`
        - `panel_01_final_v1.png`
        - `panel_01_publish_v1.jpg`
        - `panel_02_base_v1.png`
        - `panel_02_final_v1.png`
        - `panel_02_publish_v1.jpg`
        - `...`
        - `run_metadata.json`
...
예시:
- `GermanLifeCatWebtoon/2026/03/run-abc12345/panel_06_publish_v1.jpg`
...
- 파일 역할은 접두어로 구분한다.
  - `thumbnail_*_vN.(png|jpg)`: 썸네일 원본/최종본/배포용 파일
  - `panel_*_base_vN.png`: 텍스트 없는 패널 원본
  - `panel_*_final_vN.png`: 최종 검수 기준 패널 PNG
  - `panel_*_publish_vN.jpg`: 배포용 패널 JPG
  - `panel_dialogues.json`: 컷별 등장인물 대사 매핑
  - `panel_video_prompts.json`: 컷별 비디오 생성 프롬프트
  - `run_metadata.json`: 실행 메타데이터

#### 산출물 관계 정의
- `panel_*_base_vN.png`는 생성 직후의 텍스트 없는 컷 원본이다.
- `panel_*_final_vN.png`는 최종 검수 기준으로 보는 패널 PNG다.
- `panel_*_publish_vN.jpg`는 Drive 공유 및 후속 전달용 JPG다.
- `panel_dialogues.json`은 이미지와 분리된 대사 데이터다.
- `panel_video_prompts.json`은 각 패널을 비디오 생성 AI에 넘길 때 사용하는 설명 프롬프트다.
- `run_metadata.json`은 실행 상태, 품질 보고서, 파일 연결 관계를 요약한다.

#### 메타데이터 연결 규칙
- 각 산출물 JSON에는 아래 참조 필드를 포함한다.
  - `source_brief_version`
  - `source_image_version`
  - `dialogue_export_file`
  - `video_prompt_export_file`
- `run_metadata.json`은 각 실행의 최신 활성 버전과 파일 관계를 요약한다.

#### 최종 승인 체크리스트
- 캐릭터 일관성이 유지되는가
- 각 패널 이미지 안에 말풍선, 자막, 캡션이 들어가지 않았는가
- `panel_dialogues.json`의 화자와 대사 줄 매핑이 정확한가
- `panel_video_prompts.json`이 패널 장면과 소품 연속성을 정확히 설명하는가
- 주제가 요청 의도와 맞는가
- 문화적 표현이 과도하게 어색하거나 민감하지 않은가
- 최종 이미지 비율과 품질이 후속 Drive 공유 및 비디오 생성 입력용으로 적합한가

#### 승인 결과 규칙
- 사용자는 아래 셋 중 하나를 선택한다.
  - `approved`: 바로 사용 가능
  - `needs_edit`: 수동 수정 후 다시 승인 필요
  - `rejected`: 이번 실행은 사용 불가

#### 반려 처리 규칙
- `needs_edit`인 경우 `editing` 상태로 되돌린다.
- 대사나 영상 프롬프트만 수정하면 JSON 산출물만 다시 생성한다.
...
#### JSON 수정 이력 관리
- 별도 시트는 사용하지 않는다.
- 필요 시 대화 또는 영상 프롬프트 수정 이력은 `run_metadata.json` 또는 별도 JSON diff 파일로 기록한다.
  - `rerender_required`
...
#### 권한 및 리소스 관리 원칙
- Google Drive 루트 폴더는 OAuth 인증에 사용하는 본인 계정이 접근 가능해야 한다.
- 운영에 사용하는 폴더 ID는 환경변수로 주입한다.
...
9. 웹툰 생성 에이전트 실행 → 6컷 기획/캡션 초안 생성
10. 사용자가 스크립트 초안을 검토하고 승인, 재생성 요청, 직접 수정 중 하나를 선택한다.
11. 승인된 기획 기준으로 썸네일 1장과 본문 패널 6장 생성
12. 생성 결과를 주차 폴더(Drive)에 저장
13. 대화 JSON과 영상 프롬프트 JSON 생성
14. JSON 산출물을 동일 폴더에 저장
15. 사용자가 필요한 만큼 수동 수정 후 최종 승인
16. Drive 업로드 결과와 품질 상태를 `run_metadata.json`에 기록한다.
...
- 대화 또는 영상 프롬프트 JSON 생성 실패
  - JSON 구조 불일치: 1회 재생성 후 수동 검토
  - 화자 매핑 오류: 수동 확인 필요
- Drive 저장 실패
...
- 주제 중복 검사: sentence-transformers + 로컬/Drive 이력 메타데이터
- 이미지 생성: 고정 참조 이미지 지원 모델(API)
- 품질 검수: OCR 또는 Vision 기반 검사 + LLM 검토
- 저장/기록: Google Drive API + `run_metadata.json`
...
- 범위
  ...
  - 최종 주제로 6컷 기획과 캡션 초안을 생성한다.
  - 결과를 Drive와 `run_metadata.json` 구조에 기록한다.
- 완료 기준
  - 수동 주제 입력과 추천 모드가 모두 동작한다.
  - 추천 결과에 중복 점수와 제외 근거가 포함된다.
  - 기획 JSON이 정의된 스키마를 만족한다.
- 테스트 데이터
  - 기존 주제 이력 20건 이상
  - 계절성 주제 5건 이상
- 다음 단계 진입 조건
...
- 범위
  - 승인된 기획으로 텍스트 없는 컷 이미지를 생성한다.
  - 캐릭터 참조 이미지를 입력으로 사용한다.
- 완료 기준
  - 썸네일 1장 + 본문 6컷 이미지 생성 성공
  - 캐릭터 일관성 PoC 통과
...
- 완료 기준
  - 화자별 대사 매핑 JSON 생성
  - 패널별 영상 프롬프트 JSON 생성
  - 장면/소품 연속성이 프롬프트에 반영됨
- 테스트 데이터
  - 연속 소품이 포함된 샘플 10건
- 다음 단계 진입 조건
  - 화자 매핑 정확성과 프롬프트 일관성 확인 완료

### 4단계 MVP: 승인 및 Drive 산출물 확정
- 범위
  - 승인 기록
  - Drive 업로드 및 메타데이터 확정
- 완료 기준
  - 승인 상태에서만 최종 산출물 확정
  - Drive 폴더와 메타데이터 연결 확인
  - 동일 주차 실행 간 활성 실행 1개 유지 확인
- 테스트 데이터
  - 테스트 run 3회
- 다음 단계 진입 조건
  - Drive 업로드 실패, 재시도, 활성 실행 전환 시나리오 검증 완료
